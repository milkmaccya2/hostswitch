...
describe('ProcessManager', () => {
  ...
  describe('executeEditor()', () => {
    ...
    it('エディタ実行エラーを適切に処理', async () => {
      ...
      await expect(manager.executeEditor('nonexistent-editor', '/test/file.txt'))
    ...
    it('エディタが終了コード1で終了した場合も例外を投げる', async () => {
      ...
      await expect(manager.executeEditor('editor', '/test/file.txt'))
        .rejects.toThrow('Command failed: editor')
    })
